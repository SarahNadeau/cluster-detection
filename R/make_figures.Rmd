...
results_per_sample <- rbind(
    ...
    read.delim(paste0(data_dir[[2]], "/collated_results_per_sample_with_beast.txt"), sep = " ") %>% 
        mutate(outbreak = "K. aerogenes in CICU", is_focal = sample %in% focal_outbreak_samples[[2]])
    ) %>%
    mutate(method = factor(
        method, 
        levels = c("hivtrace", "clustertracker", "mugration", "beast_dta"),
...
outbreak_spread_by_method <- results_per_introduction %>%
    ...
    mutate(`Sample type` = factor(`Sample type`, levels = c("Other", "Focal region", "Outbreak")))
