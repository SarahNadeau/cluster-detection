...
params.output_folder = "results/boston_confa/clustertracker"  // where results files will be saved to
...
include { get_clustertracker_metadata_from_nextstrain } from './modules/metadata_utils.nf'
...
workflow {
    ...
    proximities = get_proximities(download_nextstrain_covid_data.out.alignment, focal_alignment, reference_fasta, params.reference_name)
    ...
    full_metadata = get_clustertracker_metadata_from_nextstrain(augur_filter.out.filtered_context_metadata, input_metadata)

    // Run clustertracker
    vcf = fasta_to_vcf(augur_filter.out.alignment_plus_filtered_context, reference_fasta)
    tree = build_tree(augur_filter.out.alignment_plus_filtered_context, params.outgroup_taxon)
